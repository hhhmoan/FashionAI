...
def learning_rate_with_decay(
  ...
  batches_per_epoch = 80#num_images / batch_size
...
def selfattention_v1(inputs):
  w = tf.get_variable(name = 'attention_dense', shape=[7,7])
  w_t = tf.reshape(tf.tile(w,[2048,1]),[2048,7,7])
  return tf.multiply(inputs, w_t)

def selfattention_v2(inputs):
  trans = tf.transpose(inputs, [0, 2, 3, 1])
  attention_dense = tf.layers.dense(trans, 1, use_bias=False)
  attention_softmax = tf.nn.softmax(tf.reshape(attention_dense,[-1,49]))
  attention_softmax = tf.reshape(attention_softmax,[-1,1,7,7])
  attention = tf.tile(attention_softmax,[1,2048,1,1])
...
def resnet_model_fn(features, labels, mode, model_class,
  ...
  for v in tf.trainable_variables():
      is_skip = False
      if v.name.find('dense') >=0 :
        is_skip = True
      if not is_skip:
        #print(v.name)
        old_name = v.name.split('/')[0] + '/'
        transfer_dict[old_name] = old_name
  #print(transfer_dict)
  tf.train.init_from_checkpoint("/home/hhh/fashionAI/FashionAI/resmodel/", transfer_dict)
...
def resnet_main(flags, model_function, input_function):
  ...
  for _ in range(flags.train_epochs // flags.epochs_per_eval):
    train_hooks = [tf.train.LoggingTensorHook(tensors=_TENSORS_TO_LOG,every_n_secs=10)]#hooks_helper.get_train_hooks(flags.hooks, batch_size=flags.batch_size)

    print('Starting a training cycle.')

    def input_fn_train():
      return input_function('train', flags.batch_size,5)
    ...
    def input_fn_eval():
      return input_function('eval', flags.batch_size, 1)

    eval_results = classifier.evaluate(input_fn=input_fn_eval)
    print(eval_results)
  def input_fn_test():
    return input_function('test', flags.batch_size, 1)
  test_results = classifier.predict(input_fn=input_fn_test)
  #print(list(test_results))
  pickle.dump(list(test_results),open('test_results.pkl','w'))
